...
- **Multi-tenant hosting** — carrying thousands of separate vaults on one
  ...
  scheme is unchanged. A1 through A4 are complete: the materializer multiplexes
  ...
  user schemas, and Redis atomically enforces a per-vault storage quota.
  Remaining work includes per-vault write limits, lifecycle, and observability.
