...
again after data-block reader search/pagination and the expanded field controls
landed. Based on reading
...
behavior. File/image and rich-text types remain absent. Date and date-time
fields are supported as ISO strings, with date-time values normalized to UTC
and displayed in the browser locale. Text properties can render as plain or
long text, URL, or email controls; unsafe URL schemes remain inert text.
