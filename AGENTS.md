...
- **Tests**: `tests/data-blocks.spec.ts`, `noteFixture()` + five new tests - edit/render round trip for every supported construct, the XSS-non-execution test that caught the bug above, unsafe/relative link degradation, image-syntax-becomes-link, and the length-cap enforcement (via real keystrokes past a `fill()`-primed near-cap value, since a raw `.value` assignment bypasses the browser's native `maxlength` enforcement and would prove nothing).
